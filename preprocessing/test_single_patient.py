...
from configs.config_preprocessing_shared import DATA_ROOT, OUTPUT_DIR, PreprocessingConfig, CHANNEL_MAP
...
def main():
    ...
    # 5. sCT channel — index 8 in both the 9-channel tensor and CHANNEL_MAP
    sct_channel = input_tensor[8]
    sct_nonzero = sct_channel[sct_channel != 0]
    print(f"\n  sCT intensity channel (index 8):")
    ...
    print(f"\n[NOTE] geometric_channels_pending = {geom_flag}")
    print(f"       Geom channels (indices 9–13) are appended separately by add_geom_channels.py.")
    ...
    print("Next step: run preprocess_all.py on the full cohort (on Snellius).")
    print("=" * 60)
